...
def main(num_threads=NUM_THREADS):
    ...
    if DO_EVAL_LSTM:
        print(f"\nEvaluating {len(lstm_models)} LSTM models on {len(dates)} days")
    ...
    if DO_EVAL_RR:
        print(f"\nEvaluating {len(rr_models)} Ridge Regression models on {len(dates)} days")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Evaluate single-day models in a multithreaded fashion')
