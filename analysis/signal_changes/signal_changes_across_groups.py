...
def create_signal_quality_across_regions_figure(calc_avg_sbp=True, calc_pr=True):
    ...
    create_avg_sbp_across_regions_plot([ax[0], ax[1], ax[2]])
...
def nothing():
    sbp_avgs = pd.read_csv(os.path.join(signal_utils.output_path, "sbp_avgs_across_regions.csv"), index_col=0)
...
def create_avg_sbp_across_regions_plot(ax_list):
    sbp_avgs = {}
    ...
    for ax, r in zip(ax_list, regions):
        ...
        ax.annotate(f'slope: {slope:.3e} \nint: {intercept:.3f}\nr^2:{R**2:.3f},\np:{p_val:.3f}', (sbp_long[r]['date_ordinal'].min(), 10), ha='left')
